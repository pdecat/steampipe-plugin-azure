...
func listKubernetesClusters(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	...
	for pager.More() {
		...
		if err != nil {
			plugin.Logger(ctx).Error("aazure_kubernetes_cluster.listAzureDataProtectionBackupJobs", "api_error", err)
			return nil, nil
		}
...
func getKubernetesCluster(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	...
	plugin.Logger(ctx).Trace("op.Properties.SupportPlan", op.Properties.SupportPlan)
